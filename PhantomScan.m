...
function [coordSphereData, coordSpherePlot, optCorrelation, sphereGoundTruth,...
    ...
    PhantomScan(volSigData, sphereData, voxelHeight,voxelWidth, voxelLength,slices,...
...
for sliceStep = 1:nSlices
    ...
    for rowStep = 1:nRows
        ...
        for colStep = 1:spheresInRow(rowStep)
            ...
            startLocation = [currentCol currentRow currentSlice];
            % Find the sphere
            [coordSphereData{saveCount},coordSpherePlot{saveCount}, optCorrelation{saveCount},...
                weightCoord{saveCount}, weightCoordPlot{saveCount},...
                coordSphereDataUp{saveCount}, coordSpherePlotUp{saveCount}, optCorrelationUp{saveCount},...
                searchIndX{saveCount},searchIndY{saveCount},searchIndZ{saveCount}] = ...
                findSphereCenter(volSigData, sphereData,startLocation,voxelHeight,voxelWidth, voxelLength,...
                searchDist,currWeightSumRange(saveCount),upScale,sphereBoundary,MRorCT);
            % ground truth locations (note rotation for plotting) [y x z]
            sphereGoundTruth{saveCount} = [gndTruthRow gndTruthCol gndTruthSlice];
            % print progress of cycling through the spheres
            if saveCount == numSpheresPercentFound(countPercentFound)
                disp(strcat([num2str(percentFound(countPercentFound)),...
                    '% of sphere locations determined']))
